document.addEventListener("DOMContentLoaded", async function () {
  try {
    const response = await fetch("http://localhost:3000/registros"); // Ruta en backend
...
function mostrarRegistros(registros) {
  const tablaCiudadanosInstituciones = document.getElementById(
    "tablaCiudadanosInstituciones"
  );
  const tablaRestaurantesLocales = document.getElementById(
    "tablaRestaurantesLocales"
  );
  ...
  registros.forEach((registro) => {
    ...
    fila.innerHTML = `
            ...
            <td>
                <button class="btn btn-warning btn-sm" onclick="editarRegistro(${
                  registro.id
                }, '${registro.tipo}')">Editar</button>
                <button class="btn btn-danger btn-sm" onclick="eliminarRegistro(${
                  registro.id
                }, '${registro.tipo}')">Eliminar</button>
            </td>
        `;
    ...
    if (registro.tipo === "ciudadano" || registro.tipo === "institucion") {
      tablaCiudadanosInstituciones.appendChild(fila);
    } else if (registro.tipo === "restaurante" || registro.tipo === "local") {
