document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function cargarDonantes() {
        try {
            ...
            selectDonante.innerHTML = '<option value="" selected disabled>Seleccione un donante</option>';
    ...
    form.addEventListener("submit", async function (e) {
        ...
        const precio = donacionVentaValue === "vendido" ? parseFloat(document.getElementById("precioAlimento").value) : null;
        const cantidad = donacionVentaValue === "vendido" ? parseInt(document.getElementById("cantidadAlimento").value) : null;
        const unidad = donacionVentaValue === "vendido" ? document.getElementById("unidadAlimento").value : null;
        const fechaCaducidad = document.getElementById("fechaCaducidad").value;
        const idDonante = document.getElementById("nombreDonante").value;

        if (!nombre || !tipo || !donacionVentaValue || !fechaCaducidad || !idDonante) {
            alert("Todos los campos obligatorios deben estar llenos.");
            return;
        }

        try {
            const response = await fetch("http://localhost:3000/api/alimentos/registrar", {
                method: "POST",
